// index.js 是命令分发中心。
  ...
  const { runChatCommand } = require("./app/chat-cli");
  ...
  Commands:
    ...
    build-dashboard 创建 dashboard
    agent-runtime   使用 OpenAI tool calling 执行多步 Agent
    chat            进入交互式聊天模式
  ...
  Examples:
    ...
    node ./bin/career-dev-agent.js agent-runtime --stdin --verbose
    node ./bin/career-dev-agent.js chat
    node ./bin/career-dev-agent.js chat --verbose
  `);
  }
